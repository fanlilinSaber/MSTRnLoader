...
//  本文件是copy react-native-update 的热修复源码修改，一些常量保持一致，后面可以支持  react-native-update 热修复正常使用

#import "MSTHotUpdate.h"
#import "MSTHotUpdateDownloader.h"
#import "MSTHotUpdateManager.h"
...
@interface MSTHotUpdate ()
{
    MSTHotUpdateManager *_fileManager;
}
@end

@implementation MSTHotUpdate

@synthesize bridge = _bridge;
@synthesize methodQueue = _methodQueue;

RCT_EXPORT_MODULE(MSTHotUpdate);
...
+ (NSURL *)bundleURL
{
    ...
    if (updateInfo) {
        NSString *curPackageVersion = [MSTHotUpdate packageVersion];
        ...
        if (needClearUpdateInfo) {
            ...
            if (loadVersioin.length) {
                NSString *downloadDir = [MSTHotUpdate downloadDir];
    ...
    return [MSTHotUpdate binaryBundleURL];
}
...
- (NSDictionary *)constantsToExport
{
    ...
    ret[@"downloadRootDir"] = [MSTHotUpdate downloadDir];
    ret[@"packageVersion"] = [MSTHotUpdate packageVersion];
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        _fileManager = [MSTHotUpdateManager new];
    }
    return self;
}
...
RCT_EXPORT_METHOD(setNeedUpdate:(NSDictionary *)options)
{
    ...
    if (hashName.length) {
        ...
        newInfo[paramPackageVersion] = [MSTHotUpdate packageVersion];
        [defaults setObject:newInfo forKey:keyUpdateInfo];
...
RCT_EXPORT_METHOD(reloadUpdate:(NSDictionary *)options)
{
    ...
    if (hashName.length) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self->_bridge setValue:[[self class] bundleURL] forKey:@"bundleURL"];
            [self->_bridge reload];
        });
    }
}
...
- (void)hotUpdate:(HotUpdateType)type options:(NSDictionary *)options callback:(void (^)(NSError *error))callback
{
    ...
    NSString *dir = [MSTHotUpdate downloadDir];
    ...
    [MSTHotUpdateDownloader download:updateUrl savePath:zipFilePath progressHandler:^(long long receivedBytes, long long totalBytes) {
        ...
        if (error) {
            ...
            [self->_fileManager unzipFileAtPath:zipFilePath toDestination:unzipFilePath progressHandler:^(NSString *entry,long entryNumber, long total) {
                ...
                // 解压进度
                [self sendEventWithName:EVENT_PROGRESS_UNZIP body:@{
                                                                    PARAM_PROGRESS_HASHNAME:hashName,
                    ...
                    if (error) {
                        ...
                        switch (type) {
                            case HotUpdateTypePatchFromPackage:
                            {
                                ...
                                NSString *bundleOrigin = [[MSTHotUpdate binaryBundleURL] path];
...
{
    NSString *unzipDir = [[MSTHotUpdate downloadDir] stringByAppendingPathComponent:hashName];
